...
class openshift_api_ {
	...
	function command($api,$command,$subcommand=false,$data=false) {
		...
		global $basicPage;
		$basicPage->writeLog($api_url.$command.($subcommand?' : '.$subcommand:''),'Input:'.$data.'<br><div onclick="$($(this).next()).toggle();" class="logklik">Response</div><br><div style="display: none;">'.$this->stdClassToString($this->response).'<div>');
	}
